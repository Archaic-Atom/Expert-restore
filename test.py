...
def test_RainDrop(net, dataset, task="raindrop"):
    output_path = testopt.output_path + task + '/'
    subprocess.check_output(['mkdir', '-p', output_path])
    ...
    with torch.no_grad():
        for ([degraded_name], degrad_patch, DA_CLIP_degrad_patch) in tqdm(testloader):
            degrad_patch = degrad_patch.cuda()
            DA_CLIP_degrad_patch = DA_CLIP_degrad_patch.cuda()

            image_features, degra_features = DA_CLIP_model.encode_image(DA_CLIP_degrad_patch, control=True)
            degra_features = degra_features.float()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_path', type=str, default="ExpertRestore_results/", help='output save path')
    ...
    if testopt.mode == 0:
        ckpt_path = "ckpt_denoise_new/" + testopt.ckpt_name
    elif testopt.mode == 1:
        ckpt_path = "final_model/" + testopt.ckpt_name
    elif testopt.mode == 2:
        ckpt_path = "ckpt_dehaze_new/" + testopt.ckpt_name
    elif testopt.mode == 5:
        ckpt_path = "final_model/" + testopt.ckpt_name
    elif testopt.mode == 7:
        ckpt_path = "ckpt_raindrop/" + testopt.ckpt_name


    net  = ExpertRestoreModel().load_from_checkpoint(ckpt_path).cuda()
    net.eval()
    ...
    if testopt.mode == 0:
        ...
        raindrop_set = TestSpecificDataset(testopt, net.DA_CLIP_preprocess)
        test_RainDrop(net, raindrop_set)
