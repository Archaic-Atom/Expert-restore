...
class ExpertRestoreTrainDataset(Dataset):
    def __init__(self, args, preprocess):
        super(ExpertRestoreTrainDataset, self).__init__()
        self.args = args
        ...
        self.de_dict = {'denoise_15': 0, 'denoise_25': 1, 'denoise_50': 2, 'derain': 3, 'dehaze': 4, 'deblur' : 5, 'enhance' : 6, 'raindrop' : 7}
    ...
    def _init_ids(self):
        ...
        if 'raindrop' in self.de_type:
            self._init_raindrop_ids()
    ...
    def _init_raindrop_ids(self):
        temp_ids = []

        image_list = os.listdir(os.path.join(self.args.raindrop_dir, 'rainy/'))
        temp_ids = image_list
        self.raindrop_ids = [{"clean_id" : x,"de_type":7} for x in temp_ids]
        self.raindrop_counter = 0
        self.num_raindrop = len(self.raindrop_ids)
        print('Total Raindrop Ids : {}'.format(self.num_raindrop))
    ...
    def _get_raindrop_name(self, raindrop_name):
        gt_name = raindrop_name.replace("rainy", "gt")
        return gt_name
    ...
    def _merge_ids(self):
        ...
        if "raindrop" in self.de_type:
            self.sample_ids += self.raindrop_ids
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                class_id = 5
                # RainDrop with NTIRE Game
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.raindrop_dir, 'rainy/', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.raindrop_dir, 'gt/', sample["clean_id"])).convert('RGB')), base=16)
                clean_name = self._get_raindrop_name(sample["clean_id"])

            degrad_patch, clean_patch = random_augmentation(*self._crop_patch(degrad_img, clean_img))

        DA_CLIP_degrad_path = self.preprocess(Image.fromarray(degrad_patch))
        clean_patch = self.toTensor(clean_patch)
        degrad_patch = self.toTensor(degrad_patch)


        return [clean_name, class_id], degrad_patch, clean_patch, DA_CLIP_degrad_path
...
class DenoiseTestDataset(Dataset):
    ...
    def __getitem__(self, clean_id):
        ...
        DA_CLIP_degra_Patch = self.preprocess(Image.fromarray(noisy_img))
        clean_img, noisy_img = self.toTensor(clean_img), self.toTensor(noisy_img)

        de_id = 0

        return [clean_name, de_id], noisy_img, clean_img, DA_CLIP_degra_Patch
...
class DerainDehazeDataset(Dataset):
    def __init__(self, args, preprocess, task="derain",addnoise = False,sigma = None):
        ...
        self.task_dict = {'derain': 0, 'dehaze': 1, 'deblur': 2, 'enhance': 3, 'desnow': 4}
        self.toTensor = ToTensor()
    ...
    def __getitem__(self, idx):
        ...
        DA_CLIP_degra_Patch = self.preprocess(Image.fromarray(degraded_img))
        clean_img = crop_img(np.array(Image.open(clean_path).convert('RGB')), base=16)

        clean_img, degraded_img = self.toTensor(clean_img), self.toTensor(degraded_img)
        degraded_name = degraded_path.split('/')[-1][:-4]

        return [degraded_name, self.task_idx + 1], degraded_img, clean_img, DA_CLIP_degra_Patch
...
class TestSpecificDataset(Dataset):
    ...
    def _init_clean_ids(self, root):
        ...
        print("Total Images Length : {%d }" % (len(name_list)))
    ...
    def __getitem__(self, idx):
        ...
        return [name], degraded_img, DA_CLIP_degra_Patch
